#!/usr/bin/env python3
"""Create a RELION 3.0 STAR file from the motor_easy GT-aligned subtomograms, and
write a dummy per-tilt CTF STAR file required by OPUS-ET's dataset loader.

Usage:
    python setup_motor_easy_opus.py <PROJ_DIR>

PROJ_DIR is where particles.star will be written (created if absent).
The dummy_ctf.star is written into the particle data directory.
"""
import glob, os, sys

PARTICLE_DIR = os.path.expanduser(
    '~/Research/synthetic_sta/motor_easy/hc_test_x6/subtomos/merged_AC_full'
)
DUMMY_CTF = os.path.join(PARTICLE_DIR, 'dummy_ctf.star')

if len(sys.argv) < 2:
    sys.exit('Usage: setup_motor_easy_opus.py <PROJ_DIR>')

PROJ_DIR = sys.argv[1]
os.makedirs(PROJ_DIR, exist_ok=True)
OUT_STAR = os.path.join(PROJ_DIR, 'particles.star')

# --- Write dummy CTF STAR into particle datadir ---
# get_3dctfs() changes _rlnCtfImage extension from .mrc -> .star and reads
# this file. With --ctfalpha 0 --ctfbeta 0 these values are never applied.
dummy_ctf_content = """\
data_images

loop_
_rlnAngleTilt
_rlnDefocusU
_rlnVoltage
_rlnSphericalAberration
_rlnAmplitudeContrast
_rlnCtfBfactor
_rlnCtfScalefactor
0.0\t20000.0\t300.0\t2.7\t0.07\t0.0\t1.0

"""
with open(DUMMY_CTF, 'w') as f:
    f.write(dummy_ctf_content)
print(f'Wrote dummy CTF STAR -> {DUMMY_CTF}')

# --- Write particles STAR ---
mrc_paths = sorted(glob.glob(os.path.join(PARTICLE_DIR, 'subtomo_*.mrc')))
if not mrc_paths:
    sys.exit(f'ERROR: No subtomo_*.mrc files found in {PARTICLE_DIR}')

lines = [
    'data_',
    'loop_',
    '_rlnImageName',
    '_rlnCtfImage',
    '_rlnAngleRot',
    '_rlnAngleTilt',
    '_rlnAnglePsi',
    '_rlnOriginX',
    '_rlnOriginY',
    '_rlnOriginZ',
    '_rlnMicrographName',
]

for p in mrc_paths:
    basename = os.path.basename(p)            # subtomo_0000.mrc
    stem     = os.path.splitext(basename)[0]  # subtomo_0000
    # _rlnCtfImage = dummy_ctf.mrc -> get_3dctfs looks for datadir/dummy_ctf.star
    lines.append(f'{basename}\tdummy_ctf.mrc\t0.0\t0.0\t0.0\t0.0\t0.0\t0.0\t{stem}')

with open(OUT_STAR, 'w') as f:
    f.write('\n'.join(lines) + '\n')

print(f'Wrote {len(mrc_paths)} particles -> {OUT_STAR}')
