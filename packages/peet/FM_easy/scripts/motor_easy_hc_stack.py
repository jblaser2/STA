...
Creates under ~/Research/peet/motor_easy_hc/results/:
  stacked.mrc               Z-stacked volume (96 x 96 x 60864 for 634 particles)
  stacked.mod               IMOD model with 634 particle positions
  motor_easy_hc_MOTL_Tom1_Iter1.csv  initMOTL (identity angles)

Also writes:
  ~/Research/STA/peet/motor_easy_hc.prm   PEET parameter file
...
ALN_DIR   = os.path.expanduser(
    "~/Research/synthetic_sta/motor_easy/hc_test_x6/subtomos/merged_AC_full")
LABELS_CSV = os.path.join(ALN_DIR, "labels.csv")

OUT_DIR   = os.path.expanduser("~/Research/peet/motor_easy_hc/results")
PEET_DIR  = os.path.expanduser("~/Research/peet/motor_easy_hc/results")
...
def main():
    ...
    motl_path = os.path.join(OUT_DIR, "motor_easy_hc_MOTL_Tom1_Iter1.csv")
    motl2_path = os.path.join(OUT_DIR, "motor_easy_hc_MOTL_Tom1_Iter2.csv")
    ...
    ref_path = os.path.join(OUT_DIR, "motor_easy_hc_initial_ref.mrc")
    prm_path = os.path.join(PEET_DIR, "motor_easy_hc.prm")
...
"""
    ...
    print(f"  # NOTE: averageAll outputs motor_easy_hc_avg_Tom1_Iter1.mrc")
    print(f"  # Copy it to the reference path expected by pca:")
    print(f"  cp motor_easy_hc_avg_Tom1_Iter1.mrc {ref_path}")
    print(f"  # Run PCA:")
