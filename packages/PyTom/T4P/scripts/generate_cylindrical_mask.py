...
def create_cylindrical_mask(box_size: int, height_pos: float, height_neg: float, radius: float) -> np.ndarray:
    """
    Create a binary cylindrical mask with asymmetric Y extent.
    ...
    A voxel at index [iz, iy, ix] is inside the cylinder when:
        ...
        AND (iy - cy) <= height_pos                   (+Y extent from center)
        AND (iy - cy) >= -height_neg                  (-Y extent from center)
    """
    ...
    in_height = ((yy - cy) <= height_pos) & ((yy - cy) >= -height_neg)
...
def main():
    ...
    parser.add_argument('--height_pos', type=float, default=8.8,
                        help='Half-extent from center in +Y direction in voxels (default: 8.8)')
    parser.add_argument('--height_neg', type=float, default=8.8,
                        help='Half-extent from center in -Y direction in voxels (default: 8.8)')
    ...
    vox = args.voxel_size
    total_height = args.height_pos + args.height_neg
    print("Cylindrical mask parameters:")
    print(f"  Box          : {args.box}^3 voxels")
    print(f"  Height +Y    : {args.height_pos} voxels  ({args.height_pos * vox:.1f} A)")
    print(f"  Height -Y    : {args.height_neg} voxels  ({args.height_neg * vox:.1f} A)")
    print(f"  Total height : {total_height} voxels  ({total_height * vox:.1f} A)")
    print(f"  Radius (XZ)  : {args.radius} voxels  ({args.radius * vox:.1f} A)")
    print(f"  Center       : ({args.box//2}, {args.box//2}, {args.box//2})")

    mask = create_cylindrical_mask(args.box, args.height_pos, args.height_neg, args.radius)

    n_voxels = int(mask.sum())
    theoretical = np.pi * args.radius ** 2 * total_height
