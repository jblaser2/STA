...
% The PCA prerotation reuses the subtomo averaging filter chain
% (initialize/refresh_subtomo_filters), which expect the supersampling and
% low-pass fields that only the averaging path (parallel_average) sets up.
% PCA prerotation never supersamples, so replicate those defaults here.
% Without this, refresh_subtomo_filters errors on the missing o.avg_ss field
% and generate_wedgemask_slices errors on the missing o.lpf field.
if ~isfield(o,'avg_ss')
    o.avg_ss = 1;
end
if ~isfield(o,'lpf')
