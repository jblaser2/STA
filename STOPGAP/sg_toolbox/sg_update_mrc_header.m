...
% Statistics
header.amin = single(min(data(:)));
header.amax = single(max(data(:)));
header.amean = single(mean(data(:)));
...
switch class(data)
    ...
    case 'half'
        header.mode = 12;
    otherwise
        error('ACHTUNG!!! Input data has unsupported data type!!! Only "int8", "int16", "float", "uint16", and "half" suppported!!!');        
end
