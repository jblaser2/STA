...
> Last updated: **2026-07-01** by Claude (TomoFlow T4P: ARI cross-package computation → reclassified non-structural; 10×10 ARI grid complete).

## Now / Next / Parked

- **T3SS INJECTISOME — ACTIVE SYNTHETIC HARD DATASET (all 10 packages complete, 2026-06-30):**
  Replaced FM_hard + FM_switch as the benchmark's second synthetic dataset (structurally distinct
  from FM_easy: different fold, different complex). 415p, 48³, class_B=IM ring present (215p) vs
  class_C=IM ring absent (120p) + 80 junk. ARI scored on signal only. Final ranking (best k):
  | Package | Best ARI | k | Notes |
  |---|---|---|---|
  | **DISCA** | **0.812** | k=3 | CNN detects ring presence/absence; immune to registration wall |
  | PEET | 0.083 | k=3 pc1_10 | Marginal — best PCA method |
  | OPUS-TOMO | 0.041 | k=3 | VAE slight improvement |
  | STOPGAP | 0.025 | k=3 | PCA k-means |
  | RELION | 0.014 | k=3 | Near-collapse |
  | PyTom | 0.009 | k=3 | Cylinder mask |
  | Dynamo | 0.000 | k=2 | Collapsed |
  | EMAN2 | 0.000 | k=3 | All-one-class |
  | TomoFlow | 0.000 | k=2 | Single cluster (24³ too small) |
  | ProTomo | −0.032 | k=2 | SVD+HAC collapsed |
  **Key finding:** Registration wall confirmed on 9/10 packages (all PCA/alignment-based collapse).
  DISCA uniquely escapes because its CNN requires no pre-registration step. Gross ring-tier
  difference (DISCA ARI=0.812) vs subtle assembly difference (FM_easy PEET ARI=0.45): CNN method
  sensitivity scales with structural magnitude while PCA sensitivity is blocked by registration.
  Scripts: `packages/<pkg>/T3SS/scripts/`. CSVs: `outputs/T3SS/<pkg>/`.
  Figures: `packages/figures/T3SS/`, generated by `scripts/eval/gen_t3ss_figures.py`.
  **NEXT:** Write analysis section on DISCA vs PCA registration-wall tradeoff; consider running
  T3SS on aligned particles to mirror FM_easy alignment experiment; or move to T4SS dataset.

- **TOMOFLOW T4P — RECLASSIFIED NON-STRUCTURAL (2026-07-01):**
  Masked run (ORC SLURM job 12460059, cyl v2, k=2 → 403/269) ARI computed vs all packages.
  ARI = −0.001 vs Dynamo / +0.001 vs PEET, BUT **ARI = 1.000 vs DISCA / 0.993 vs EMAN2 / 0.887 vs OPUS**.
  TomoFlow's masked OF landscape finds the same contrast/intensity axis as the non-structural group.
  Reclassified from "collapsed" → **non-structural**. Per-particle CSV: `results/T4P/tomoflow_k2_std.csv`.
  Standard class-avg panel: `packages/figures/T4P/tomoflow_class_avgs_std.png`.
  All-10-package grid (`packages/figures/T4P/all_pkg_grid.png`) is now a true **10×10 matrix** (no sidebar).
  Files: tomoflow/README.md, tomoflow/T4P/results/RESULTS.md, research.md, packages/README.md,
  gen_t4p_full_pkg_grid.py, gen_t4p_class_avg_panels.py.
  **NEXT:** TomoFlow adds a 4th vote to the contrast-axis group — consider updating the T4P no-GT
  evidence chain (§12 benchmarkIdeas.md) and the "Ground Truth" bullet below.
...
- **T4P EVAL FRAMEWORK — STANDARDISED (2026-06-29):** All T4P result CSVs normalised to
  ...
  **NEXT:**
  ...
  - Run T3SS on aligned particles (mirror FM_easy alignment experiment; expect DISCA to hold, PCA to improve)
...
- **FM_hard ARCHIVED (superseded by T3SS, 2026-07-01):** 3-class flagellar-motor assembly-intermediate
  dataset (813p, 96³, base/basal_body/mature, no junk). All 10 packages ran; results committed. Superseded
  because T3SS provides better structural diversity (different complex, different fold). FM_hard data and
  scripts remain in `~/Research/synthetic_sta/motor_hard/` (local) and `outputs/FM_hard/`, `packages/<pkg>/FM_hard/`
  (repo). Package matrix was removed from `packages/README.md`; historical results in `results/synthetic_scores.csv`
  under tag `*_ABC_hard_x6_813`. Key historical result: TomoFlow ARI=0.223 (only package above chance);
  all 9 PCA methods confirmed registration wall (supervised ceiling 0.472 >> best blind 0.078).
...
- **GROUND TRUTH (Stefano consult, 2026-06-01; updated 2026-06-11):** this T4P dataset has **two distinct, obvious pili-phase classes**. Packages now fall into **three groups** (see `packages/figures/T4P/cross_pkg_correlation.png`):
  ...
  - **No split / collapse:** **RELION** (672/0, algorithm-level SNR failure, all 6 configs).
  - **Non-structural (contrast axis): DISCA / EMAN2 / OPUS / TomoFlow** — all find the same contrast/intensity split (pairwise ARI 0.88–1.00), uncorrelated with conformational split (ARI≈0 vs converging group).
  - Benchmark signal: alignment/CC-based methods (Dynamo/PEET/PyTom/ProTomo) recover the conformational axis; learned-feature + OF methods (OPUS/DISCA/EMAN2/TomoFlow) default to a contrast axis; RELION soft-EM collapses. Three-group structure now confirmed by full 10×10 ARI grid.
...
| TomoFlow | ✅ | `tomoflow` | ✅ `tomoflow_run.py` | ✅ | ✅ | ✅ | — | also ContinuousFlex, but (unlike MDTOMO/HEMNMA) needs only a subtomogram-average reference. Required porting farneback3d off CUDA texture-refs for CUDA 13.2/sm_120. **T4P masked run (ORC SLURM 12460059, cyl v2): k=2 → 403/269. Reclassified NON-STRUCTURAL: ARI=1.000 vs DISCA, 0.993 vs EMAN2 — OF captures contrast/intensity axis, not conformational.** Per-particle CSV: `results/T4P/tomoflow_k2_std.csv`. 10×10 grid complete. |
