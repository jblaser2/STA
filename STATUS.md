...
> Last updated: **2026-06-04** by Claude (motor_easy all three classes complete + GT separability validated).

## Now / Next / Parked

- **SYNTHETIC DATA — motor_easy ALL CLASSES DONE (2026-06-04):** Class A: 246 subtomos, Class B: 211 subtomos, Class C: 177 subtomos. All GT-aligned averages computed + verified. **GT separability validated:** CC-template matching on GT-aligned subtomos gives ARI=0.289 (~68-73% per-class accuracy on noisy particles); class average CCs: A-B=0.72, A-C=0.66, B-C=0.83 — confirming classes are structurally distinct. Simple PCA k-means gives ARI≈0 (expected: requires known templates). All merged into `merged_all/` (634 subtomos) and `merged_all_aln/` (634 GT-aligned). Scripts: `align_all_classes.py`, `avg_gt_classC.py`, `visualize_classC.py` in `~/Research/synthetic_sta/motor_easy/`. **Next: run motor_easy through the benchmark packages (RELION, Dynamo, PEET, etc.) and compute ARI/NMI vs GT labels from `merged_all/labels.csv`.** Pipeline quirks: `STA/etsimulation/pipeline_quirks.md` (11 quirks documented, `reconstruct_metadata.py` handles sim_metadata rebuild, quirk #11).
