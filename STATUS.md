...
> Last updated: **2026-06-04** by Claude (PEET T4P classification with cylindrical masks; comparison figures vs Stefano).
...
- **DYNAMO motor_easy — BLOCKED (2026-06-04):** Dynamo k-means/MRA on motor_easy is blocked until MATLAB Parallel Computing Toolbox (PCT) is installed. PCT is licensed but not installed. This also blocks real-T4P Dynamo MRA re-runs. Fix: install PCT from MATLAB installer or mlm_install.

- **SYNTHETIC DATA — motor_easy ALL CLASSES DONE (2026-06-04):** Class A: 246 subtomos, Class B: 211 subtomos, Class C: 177 subtomos. All GT-aligned averages computed + verified. **GT separability validated:** CC-template matching on GT-aligned subtomos gives ARI=0.289 (~68-73% per-class accuracy on noisy particles); class average CCs: A-B=0.72, A-C=0.66, B-C=0.83 — confirming classes are structurally distinct. All merged into `merged_all/` (634 subtomos) and `merged_all_aln/` (634 GT-aligned). Pipeline quirks: `STA/etsimulation/pipeline_quirks.md`.
...
- **Synthetic — motor_easy (3-class flagellar motor, ~30 Å differences):** All 3 classes complete, 634 GT-aligned subtomos in `merged_all_aln/`. GT separability validated (template CC ARI=0.289). First benchmark runs: RELION ARI≈0.005, PEET WMD-PCA ARI≈0 (both miss classes — expected for these settings). Scoring infra: `scripts/eval/`. `~/Research/synthetic_sta/motor_easy/production/`.
