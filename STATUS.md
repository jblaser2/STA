...
> Last updated: **2026-06-05** by Claude (README figures: class maps regenerated with C_noRodHook + centroid-adaptive slicing; all 3 GT-aligned averages added; class C particles simulated + avg computed).
...
- **DYNAMO motor_easy — HAC RUN COMPLETE (2026-06-05):** HAC (CC-matrix Ward linkage) on 694 GT-aligned particles (96³, RELION solvent mask r=32 Y-10). CC matrix computed in 198s. **Cophenetic = 0.094** (no hierarchical CC structure). k=2: ARI=0.005; k=3: ARI=−0.009. Scores in `results/synthetic_scores.csv`; assignments in `dynamo/dynamo_outputs/motor_easy_hac/`. **Diagnosis:** per-particle SNR too low for pairwise CC to recover structure (same root cause as RELION blind run). All 177 class C particles collapse into the large cluster at k=3 without forming a pure class. Scripts: `dynamo/dynamo_scripts/dynamo_motor_easy_hac.m`, `run_motor_easy_hac.sh`. **Next: Dynamo PCA approach (dpkpca on pre-aligned particles) may perform better — see dtutorial ARI=0.878 result; or accept HAC≈0 as canonical result.**
...
- **Synthetic — motor_easy (3-class flagellar motor, ~30 Å differences):** **CLASS C RE-SIMULATED (2026-06-05)** — C_noRodHook = C-ring only (CUT2_C=46.5 base px). 5 runs, 177 particles, confirmed via `avg_gt_classC.py` (177 particles accumulated). GT-aligned avg: `production/subtomos/avg_classC_aligned.mrc`. **Current `merged_all_aln/` still uses OLD class C definition** — must run `align_all_classes.py` before next package runs (then rebuild RELION/PEET star/prm inputs). Prior RELION/PEET scores (old C_core): RELION ARI=0.380 iter1, PEET ARI=0.116 k=2. Mask for classification: r=32 px, center=(48,38), 96³ box. Scoring infra: `scripts/eval/`. `~/Research/synthetic_sta/motor_easy/production/`.
