function cc_map = calculate_flcf(mref,mask,conjTarget,conjTarget2,fmask_in)
%% calculate_flcl
...
% fmask_in    - (optional) pre-computed fftn(mask). When provided the
%               internal fftn(mask) call is skipped (saves one FFT per
%               angle evaluation for rotation-invariant masks).
%
% WW 02-2018
...
sigma_a = real(ifftn(fmask.*conjTarget2)./n_pix);       % First part of denominator sigma
sigma_b = real(ifftn(fmask.*conjTarget)./n_pix).^2;     % Second part of denominator sigma
denominator = n_pix.*sqrt(sigma_a-sigma_b);
clear sigma_a sigma_b fmask
